...
class SkippedLayerMetricLearningChallenge(MicroChallenge):
    def __init__(self, hparams):
        ...
        if "pretrained_embedding_path" in hparams and self.hparams.get("num_input_neighbors", 0) > 0:
            self.pretrained_embedding = SkippedLayerMetricLearningChallenge.load_from_checkpoint(
                hparams["pretrained_embedding_path"]
        )
            if self.hparams.get("freeze_pretrained_embedding", True):
    ...
    def get_input_data(self, batch):
        ...
        if self.hparams.get("num_input_neighbors", 0) > 0:
            input_data = torch.cat(
                [input_data, self.get_input_neighbor_data(batch)],
                dim=-1,
            )

        return input_data

    def get_input_neighbor_data(self, batch):

        embedding_input = neighbor_input_features = self.pretrained_embedding.get_input_data(batch)
        neighbor_search_features = self.pretrained_embedding(
            embedding_input
        )

        # Get the KNN graph
        knn_edges = knn_graph(
            neighbor_search_features, self.hparams.get("num_input_neighbors", 0), loop=False
        )

        # Convert from (2, E) to (N, k) where N is the number of nodes and k is the number of neighbors
        # 1. Sort in ascending order
        sorted_indices = torch.argsort(knn_edges[1])
        knn_edges = knn_edges[:, sorted_indices]

        # Reshape to (N, k)
        knn_indices = knn_edges.reshape(2, -1, self.hparams.get("num_input_neighbors", 0))[0]

        # Get the features of the neighbors
        knn_features = neighbor_input_features[knn_indices].flatten(-2, -1)
...
class BasicMetricLearningDataset(Dataset):
    ...
    def __init__(self, num_events=None, hparams=None, input_dir=None, data_name=None, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.detector = Detector(dimension=2, hole_inefficiency=1).add_from_template('barrel', min_radius=0.5, max_radius=3, number_of_layers=10)
...
from . import models
model_resolver = class_resolver.Resolver(classes = [models.MLP, models.KNN_MLP], base = torch.nn.Module)